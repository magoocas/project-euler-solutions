...
{
    public class Solution059 : SolutionBase
    {
        private bool TryGetPass(int[] cipherText, string[] guessWord, out int[] pass)
        {
            pass = null;
                ...
                for (int p2 = 'a'; p2 <= 'z'; p2++)
                {
                    for (int p3 = 'a'; p3 <= 'z'; p3++)
                    {
                        ...
                        for (int j = 0; j < cipherText.Length; j++)
                            plainText[j] = (char)(cipherText[j] ^ pass[j % 3]);
                        var plainTextString = new string(plainText);
                        foreach (var guess in guessWord)
                        {
                            if (Regex.IsMatch(plainTextString, guess))
                                return true;
                        }
                    }
                }
        ...
        {
            var cipherText = ProblemData.Split(',').Select(int.Parse).ToArray();
            
            int[] pass;
            if (!TryGetPass(cipherText, new [] {" the "}, out pass))
                return 0;

            var plainText = new char[cipherText.Length];
            for (int i = 0; i < cipherText.Length; i++)
                plainText[i] = (char)(cipherText[i] ^ pass[i%3]);

            //Console.WriteLine($"Pass: {new string(pass.Select(i=>(char)i).ToArray())}");
            //Console.WriteLine($"PlainText: {new string(plainText)}");

            return plainText.Sum(c=>c);
